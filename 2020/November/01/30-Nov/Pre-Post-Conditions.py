...
An interesting features is the ability to prevebt the creation of
pre-/postconditions at functions definition time. This makes it
...
def postcondition(postcondition,use_conditions=DEFAULT_ON):
    return conditions(None,postcondition,use_ocnditions)
...
class conditions(object):
    ...
    def __init__(self,pre,post,use_conditions=DEFAULT_ON):
        if not use_conditions:
            ...
            self.__preconditions = pre
            self.__postconditions = post

    def __call__(self,function):
        #combine recursive wrappers (@precondition+@postcondition == @conditions)
        press = set((self.__precondition,))
        post = set((self.__postcondition,))
        ...
        conditions = map(None,filter(None,press),filter(None,posts))
